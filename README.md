...
## Polymer Starter Kit + Angular2

> Demonstration of the state of Angular2 + Polymer.

### TL;DR

Ron Dodson presented his [findings and detailed setup steps](https://www.youtube.com/watch?v=7WgEuNZCCHk) at AngularU in San Francisco in June. 
In a nutshell: With some hacks, Chrome works pretty well. Safari and Firefox won't even load.
   
## Getting Started
   
Follow the Getting Started described by the Polymer Starter Kit, below. Between steps 2 and 3 you'll need to install the
typescript compiler and typescript definition files. These steps come from (https://angular.io/docs/js/latest/quickstart.html)[https://angular.io/docs/js/latest/quickstart.html], with slight modification. 
...
Prior to running `gulp serve` you'll want to compile the typescript file(s):

```sh
$ tsc --watch -m commonjs -t es5 --emitDecoratorMetadata --experimentalDecorators app/my-app.ts
```
...
JSPM is present in the package.json dependency list, but probably isn't required. Consider it a bit of wishful thinking / preview of things to come.
 
All credit goes to the Polymer Starter Kit project and Rob Dodson. 
